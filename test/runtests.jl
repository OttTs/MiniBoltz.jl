...
@testset "MiniBoltz.jl" begin
    ...
    @testset "Particle relaxation" begin
        ...
        v_standard, v_exact, v_midpoint, v_usp = copy(v), copy(v), copy(v), copy(v)
        nsteps = 100
        for _ in 1:nsteps
            Δt = τ/4 / nsteps
            ...
            Σ_iter = cov(v_usp)
            MiniBoltz.@batch for i in eachindex(v_usp)
                v_usp[i] = MiniBoltz.relax(v_usp[i], Δt, species, n, μ, Σ_iter, USPESFP())
            end
        end
        @test cov(v_standard) ≈ Σ_exact atol=2000
        @test cov(v_exact) ≈ Σ_exact atol=2000
        @test cov(v_midpoint) ≈ Σ_exact atol=2000
        @test cov(v_usp) ≈ Σ_exact atol=2000
    end
